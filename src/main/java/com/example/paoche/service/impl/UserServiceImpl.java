...
public class UserServiceImpl implements UserService {
    ...
    public LoginRespVO login(LoginReqVO loginReqVO) {
        ...
        if(user.getPassword() != Md5HashUtil.hashPassword(loginReqVO.getPassword(),user.getSalt(),1024)){
            throw new BusinessException(BaseResponseCode.ACCOUNT_PASSWORD_ERROR);
        }
        LoginRespVO loginRespVO = new LoginRespVO();
        Map<String,Object> claims = new HashMap<>();
        claims.put("roles",roleDao.findRolesByUserId(user.getId()));
        ...
        claims.put("jwt-user-name-key",user.getId());
        ...
        redisService.expire(accessToken,1, TimeUnit.DAYS);
        redisService.expire(refreshToken,1, TimeUnit.DAYS);
        return loginRespVO;
    }
}
